...
from configs.app_config import REDIS_DATABASE_URI, MONGODB_URI, SERVER_HOST, SERVER_PORT, SECRET_KEY, API_DOCS_EXPORT_PATH
from dbs.mongodb import init_mongodb
from controllers.api_design_controller import init_api_design_controller
from loggers import logger
from views.api_design_api import blp as api_design_blp
...
def create_app(app):
    ...
    app.config["API_TITLE"] = "API Design REST API"
    ...
    # 创建导出目录
    os.makedirs(API_DOCS_EXPORT_PATH, exist_ok=True)
    
    # 初始化API设计控制器
    init_api_design_controller(db_instance)
    ...
    # 注册蓝图
    api = Api(app)
    api.register_blueprint(api_design_blp)
...
if __name__ == "__main__":
    app = create_app(app)
    print("===================API Design Service starting============================")
